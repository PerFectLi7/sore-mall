<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="奖箱ID" prop="boxId" >
        <input />
          <template slot-scope="scope">
              <span>{{scope.row.boxId}}</span>
          </template>
      </el-table-column>
      <el-table-column align="center" label="奖箱名称" prop="boxName" >
        <input />
          <template slot-scope="scope">
...
<script>
import Vue from 'vue'
import { boxProductByBid } from '@/api/Box'
import { updateBoxProduct, createBoxProduct, deleteBoxProduct} from '@/api/BoxProduct'
...
export default {
  ...
  methods: {
    getList() {
      ...
      boxProductByBid(this.listQuery)
        .then(response => {
          ...
          this.boxId = response.data.data.boxProductDTOPage.items[0].boxId
          this.boxName = response.data.data.boxProductDTOPage.items[0].boxName
