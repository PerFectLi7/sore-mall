<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.name" clearable class="filter-item" style="width: 200px;" placeholder="输入奖箱名称" />
      <el-input v-model="listQuery.count" clearable class="filter-item" style="width: 200px;" placeholder="输入奖品数量" />
      <el-select v-model="listQuery.status" clearable style="width: 200px" class="filter-item" placeholder="选择奖箱状态" >
        <el-option v-for="(item,index) in boksStatusMap" :key="index" :label="item.name" :value="item.value" />
      </el-select>
    ...
    >
      <el-table-column align="center" label="奖箱ID" prop="id" >
      ...
      <el-table-column align="center" label="奖品数量" prop="count" />
      <el-table-column align="center" label="奖箱状态" prop="status">
        <template slot-scope="scope">
          <el-tag
      ...
      <el-table-column align="center" label="操作" width="350" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['product:product:detial']" type="primary" size="mini" @click="handleInfo(scope.row.id)">详情</el-button>
          </div>
          <div v-show="scope.row.show">
...
<script>
import Vue from 'vue'
import { listBox, deleteBox, editBox, createBox} from '@/api/box'
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      listLoading: true,
      boksStatusMap: [{ value: 0, name: '未满' }, { value: 1, name: '已满' }],
      listQuery: {
        page: 1,
        limit: 20,
        name: undefined,
