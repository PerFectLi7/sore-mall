...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public UserDTO getOpenId(String nickName, String avatarUrl, Integer sex, Long shareUserId, String raw) throws ServiceException, IOException {
        //微信第三方登录
        JSONObject thirdPartJsonObject = JSONObject.parseObject(raw);
        String code = thirdPartJsonObject.getString("code");
        String body = okHttpClient.newCall(new Request.Builder()
                .url("https://api.weixin.qq.com/sns/jscode2session?appid=" + (UserLoginType.MP_WEIXIN.getCode() == 1 ? this.unimallWxProperties.getMiniAppId() : this.unimallWxProperties.getAppId()) +
                        "&secret=" + (UserLoginType.MP_WEIXIN.getCode() == 1 ? this.unimallWxProperties.getMiniAppSecret() : this.unimallWxProperties.getAppSecret()) +
                        "&grant_type=authorization_code&js_code=" + code).get().build()).execute().body().string();
        JSONObject jsonObject = JSONObject.parseObject(body);
        Integer errcode = jsonObject.getInteger("errcode");
        if (errcode == null || errcode == 0) {
            String miniOpenId = jsonObject.getString("openid");
            List<UserDO> userDOS = userMapper.selectList(new QueryWrapper<UserDO>().eq("open_id", miniOpenId).eq("login_type", 1));
            UserDO userDO;
            if (CollectionUtils.isEmpty(userDOS)) {
                //若用户为空，则注册此用户
                Date now = new Date();
                UserDO newUserDO = new UserDO();
                newUserDO.setLoginType(1);
                newUserDO.setOpenId(miniOpenId);
                newUserDO.setGmtLastLogin(now);
                newUserDO.setNickname(nickName);
                newUserDO.setAvatarUrl(avatarUrl);
                newUserDO.setGender(sex);
