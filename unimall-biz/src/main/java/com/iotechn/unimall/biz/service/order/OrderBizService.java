...
import com.iotechn.unimall.data.mapper.OrderNewMapper;
...
public class OrderBizService {
    ...
    public OrderNewDTO getOrderDetail(Long orderId, Long userId) throws ServiceException {
        QueryWrapper<OrderNewDO> wrapper = new QueryWrapper<OrderNewDO>()
                .eq("id", orderId);
        if (userId != null) {
            wrapper.eq("user_id", userId);
        }
        List<OrderNewDO> orderDOS = orderNewMapper.selectList(wrapper);
        if (CollectionUtils.isEmpty(orderDOS)) {
            throw new AppServiceException(ExceptionDefinition.ORDER_NOT_EXIST);
        }
        OrderNewDTO orderNewDTO = new OrderNewDTO();
        BeanUtils.copyProperties(orderDOS.get(0), orderNewDTO);
